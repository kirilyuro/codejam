...
//    with GlobalExecutionContext
{
  ...
  val (n, k) = read[Int, Int]

  override def writeResult(): Unit = {
    writeCase(caseNumber, result match {
      case None => "IMPOSSIBLE"
      case Some(_) => "POSSIBLE"
    })

    result.foreach(_.foreach(row => writeLine(row.mkString(" "))))
  }

  lazy val result: Option[List[List[Int]]] = {
    var res: Option[List[List[Int]]] = None
    (1 to n).permutations.map(_.toList).map(perm => {

    })

//    var res: Option[List[List[Int]]] = None
//    (1 to n).permutations.map(_.toList).find(perm => {
//      val mat = perm +: (1 until n).map(shift => perm.shiftRight(shift)).toList
//
//      val trace = (0 until n).map(i => mat(i)(i)).sum
//      if (trace == k) {
//        res = Some(mat)
//        true
//      }
...
trait Utils {
  ...
  implicit class ListOps[T](value: List[T]) {
    def shiftRight(n: Int): List[T] =
      value.takeRight(n) ::: value.take(value.length - n)

    def shiftLeft(n: Int): List[T] = shiftRight(value.length - n)
  }
