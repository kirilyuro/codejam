...
//    with GlobalExecutionContext
{
  ...
  val n: Int = readInt
  val activities: List[Activity] = (1 to n).map(i => {
    val (s, e) = read[Int, Int]
    Activity(i, s, e, None)
  }).toList.sortBy(_.to)
  ...
  lazy val result: String = computeResult
  ...
  def computeResult: String = {
    ...
    for (a <- activities) {
      val available = List(c, j).filter(_.lastT <= a.from)
      a.assign = available match {
        case Nil => return "IMPOSSIBLE"
        case _ =>
          val assignedParent = available.maxBy(_.lastT)
          assignedParent.lastT = a.to
          Some(assignedParent)
      }
    }

    activities.sortBy(_.i).map(_.assign.get.name).mkString
  }

  case class Activity(i: Int, from: Int, to: Int, var assign: Option[Parent])
  case class Parent(name: Char, var lastT: Int)
}
