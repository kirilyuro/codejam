...
//    with GlobalExecutionContext
{
  ...
  val n: Int = readInt
  val matrix: List[List[Int]] = (1 to n).map(_ => readMany[Int]).toList

  override def writeResult(): Unit = {
    writeCase(caseNumber, result.k, result.r, result.c)
  }

  lazy val result: Result = {
    val k: Int = (0 until n).map(i => matrix(i)(i)).sum
    val r: Int = (0 until n).map(row => {
      mutable.Map((1 to n).map(_ -> 0):_*).tap(map => {
        (0 until n).foreach(j => map(matrix(row)(j)) = map(matrix(row)(j)) + 1)
      }).exists(_._2 > 1)
    }).count(identity)

    val c: Int = (0 until n).map(col => {
      mutable.Map((1 to n).map(_ -> 0):_*).tap(map => {
        (0 until n).foreach(i => map(matrix(i)(col)) = map(matrix(i)(col)) + 1)
      }).exists(_._2 > 1)
    }).count(identity)
  ...
  case class Result(k: Int, r: Int, c: Int)
}
