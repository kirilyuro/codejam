...
using Globs = PowerArrangers.Program.GlobalParams;

namespace PowerArrangers
{
	class Solver : SolverBase<int, char>
	{
		public const int INPUT_BUFFER_SIZE = 1024 * 1024 * 5; // 5 MB
		public static readonly string HALT_ON_EXCHANGE_RESPONSE = "N";

		public Solver(int caseNumber, Input input, Output output)
			: base(caseNumber, input, output)
		{
			output.ResetMaxExchanges(Globs.F);
		}
		...
		void FillTest(List<string> t, string curr)
		{
			if (curr.Length == 5)
			{
				t.Add(curr);
			...
			var ps = new char[] { 'A', 'B', 'C', 'D', 'E' }.Except(curr);
			foreach (var item in ps)
			{
				FillTest(t, curr + item);
			}
		...
		{
			output.WriteLine(_res);
			input.ReadString();
		}

		protected override char DoExchange(int query)
		{
			//return test[query - 1];
			output.WriteLine(query, true);
			return input.ReadString()[0];
		}
		...
		public override void Solve()
		{
			...
			for (int i = 0; i < 4; i++)
			{
				p[i] = c.Select(cc => cc[i]).GroupBy(x => x).Where(g => g.Count() != 24).Select(x => x.Key).ToArray();
			}
			...
			for (int i = 0; i < 4; i++)
			{
				if (p[i].Length > 1)
				{
					p[i] = p[i].Except(new char[] { Exchange(118 * 5 + i + 1) }).ToArray();

					for (int j = i + 1; j < 4; j++)
					{
						if (p[j].Contains(p[i].First()))
						{
							p[j] = p[j].Except(new char[] { p[i].First() }).ToArray();
						}
					}
				}
			}

			var ps = p.Select(x => x.First());
			char p5 = new char[] { 'A', 'B', 'C', 'D', 'E' }.Except(ps).First();
			_res = string.Join("", ps) + p5;
		}
	}
	...
	class Program
	{
		...
		{
			input.ReadMany(out Globs.T, out Globs.F);
		}
