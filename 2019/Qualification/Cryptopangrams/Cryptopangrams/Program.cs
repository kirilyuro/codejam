...
namespace CodeJamTemplate1
{
    class Solver
    {
        ...
        {
            var nl = _in.ReadMany("di");
            _n = (double)nl[0];
            _l = (int)nl[1];
            _ct = _in.ReadMany<double>();
        }
        ...
        {
            double a, b;
            FactorSemiPrime(_ct[0], out a, out b);
            double[] codes = new double[_l + 1];
            codes[0] = a;
            codes[1] = b;
            if (!FillCodes(codes))
            {
            ...
            var lettersMap = codes.Distinct().OrderBy(code => code).Zip(Enumerable.Range(0, 26), (code, letterIndex) => new
            {
                Code = code,
                Letter = (char)('A' + letterIndex)
            }).ToDictionary(mapping => mapping.Code, mapping => mapping.Letter);

            _y = new string(codes.Select(code => lettersMap[code]).ToArray());
        }

        private bool FillCodes(double[] codes)
        {
            for (int i = 2; i < codes.Length; i++)
            {
                codes[i] = _ct[i - 1] / codes[i - 1];
                if (!IsWhole(codes[i]))
                {
                    return false;
                }
        ...
        private static void FactorSemiPrime(double pq, out double p, out double q)
        {
            if (pq % 2 == 0)
            {
                p = 2;
            ...
            double a = Math.Ceiling(Math.Sqrt(pq));
            double b2 = a * a - pq;
            while (!IsWhole(Math.Sqrt(b2)))
            {
                a++;
                b2 = a * a - pq;
            }
        ...
        private static bool IsWhole(double n)
        {
            return Math.Abs(Math.Floor(n) - n) < double.Epsilon;
        }
    }
