...
namespace CodeJamTemplate1
{
	class Solver : SolverBase
	{
		...
		{
			input.ReadMany(out _n, out _k);
			_ci = input.ReadMany<int>();
			_di = input.ReadMany<int>();
		...
		private int Diff(int l, int r)
		{
			int maxCi = -1;
			int maxDi = -1;
			for (int i = l; i <= r; i++)
			{
				if (_ci[i] > maxCi) maxCi = _ci[i];
				if (_di[i] > maxDi) maxDi = _di[i];
			}

			return Abs(maxCi - maxDi);
		}
	}
