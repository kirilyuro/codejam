...
namespace CodeJamTemplate1
{
	class Solver : SolverBase
	{
		...
		{
			input.ReadMany(out _p, out _q);
			_coords = Enumerable.Range(0, _p).Select(i =>
			{
				var coord = new Coord();
				input.ReadMany(out coord.X, out coord.Y, out coord.D);
				return coord;
			}).ToArray();
		}
		...
		{
			output.WriteCaseResult(caseNumber, _x, _y);
		}
		...
		private int CountHeading(int i, int j)
		{
			return _coords.Count(coord =>
			{
				switch(coord.D)
				{
					case Direction.N:
