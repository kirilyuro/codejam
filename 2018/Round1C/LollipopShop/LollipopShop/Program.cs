...
namespace LollipopShop
{
	class Solver
	{
		...
		public void WriteResult(int flavorId = -1)
		{
			_out.WriteLine("i", flavorId);
		}

		private void Exchange()
		{
			var favoriteFlavors = _in.ReadMany<int>().Skip(1).ToArray();

			foreach (var flavor in favoriteFlavors)
			{
				_guess[flavor] *= 2;
			}

			var availableFlavors = favoriteFlavors.Where(flavor => !_soldOut[flavor]);
			...
			var flavorToSell = availableFlavors.MinBy(flavor => _guess[flavor]).First();
			WriteResult(flavorToSell);
			_soldOut[flavorToSell] = true;
		}

		public void Solve()
		{
			_guess = Enumerable.Repeat(0.1, _n).ToArray();
			_soldOut = Enumerable.Repeat(false, _n).ToArray();

			for (int exchange = 1; exchange <= _n; exchange++)
			{
