...
namespace AntStack
{
	class Solver
	{
		...
		public void Solve()
		{
			//_maxAnts = SolveAux(0, 0, 0);
			_maxAnts = SolveAux2();
		}

		private int SolveAux2()
		{
			var stacks = new List<AntStack>();

			for (int i = 0; i < _n; i++)
			{
				var newStacks = new List<AntStack>();

				foreach (var stack in stacks)
				{
					if (_w[i] * 6 >= stack.Weight)
					{
						newStacks.Add(new AntStack { NumAnts = stack.NumAnts, Weight = stack.Weight });
						stack.NumAnts++;
						stack.Weight += _w[i];
					}
				}

				stacks.AddRange(newStacks);
				stacks.Add(new AntStack { NumAnts = 1, Weight = _w[i] });
			}

			return stacks.Max(x => x.NumAnts);
		}
		...
		private int _maxAux = 0;

		private int SolveAux(int currAnts, long currWeight, int i)
		{
			if (i == _n) return currAnts;
			if (currAnts + _n - i < _maxAux) return -1;

			bool canAddCurr = _w[i] * 6 >= currWeight;

			if (canAddCurr && currAnts + 1 > _maxAux) _maxAux = currAnts + 1;

			var m1 = canAddCurr ? SolveAux(currAnts + 1, currWeight + _w[i], i + 1) : -1;
			var m0 = SolveAux(currAnts, currWeight, i + 1);
