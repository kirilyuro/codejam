...
namespace AWholeNewWord
{
	class Solver
	{
		...
		public void ReadData()
		{
			...
			_w = Enumerable.Range(1, _n).Select(x =>_in.ReadString()).ToArray();
		}

		public void WriteResult()
		{
			_out.WriteCaseResult(_number, "s", _result);
		}
		...
		public void Solve()
		{
			if (_l == 1 || _n == 1)
			{
				_result = ((char)45).ToString();
				return;
			}

			var distinctLetters = _w.SelectMany(x => x).Distinct().Count();
			if (_n >= Math.Pow(distinctLetters, _l))
			{
				_result = ((char)45).ToString();
				return;
			}

			if (!SolveAux("")) _result = ((char)45).ToString();
		}

		private bool SolveAux(string partialSol)
		{
			if (partialSol.Length == _l)
			{
				if (IsNewWord(partialSol))
				{
					_result = partialSol;
					return true;
				}
			...
			var distinctLetters = _w.Select(x => x[partialSol.Length]).Distinct().ToList();

			foreach (var letter in distinctLetters)
			{
				if (SolveAux(partialSol + letter)) return true;
			}
