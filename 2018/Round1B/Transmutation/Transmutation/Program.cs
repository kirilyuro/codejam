...
namespace Transmutation
{
	class Solver
	{
		...
		{
			_out.WriteCaseResult(_number, "i", result);
		}
		...
		public int MaxOf(int i)
		{
			var r = this.r[i];
			if (r[0] == 1 || r[1] == 1) return g[i];

			var m1 = MaxOf(r[0]-1);
			var m2 = m1 > 0 ? MaxOf(r[1]-1) : 0;

			return g[i] + Math.Min(m1, m2);
		}
	}
