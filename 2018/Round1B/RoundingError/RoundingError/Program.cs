...
namespace RoundingError
{
	class Solver
	{
		...
		private List<IndexedValue<int>> ci;

		private int maxRoundedValue;
		...
		public void ReadData()
		{
			...
			ci = _in.ReadMany<int>().Index().ToList();
		}

		public void WriteResult()
		{
			_out.WriteCaseResult(_number, "i", maxRoundedValue);
		}

		public void Solve()
		{
			var minPart = DecimalPart(1);
			var partsOverHalf = Math.Ceiling(0.5 / minPart);
			var answered = ci.Sum(x => x.Value);
			var remainingAnswers = n - answered;

			var alreadyOverHalf = ci.Count(c => DecimalPart(c.Value) > 0.5);

			var ici = ci.Where(x => DecimalPart(x.Value) < 0.5).ToList();

			for (int i = 0; i < remainingAnswers;)
			{
				IndexedValue<int> curr;
				if (ici.Count == 0)
				{
					curr = new IndexedValue<int>(ici.Count, 0);
					ici.Add(curr);
					ci.Add(curr);
				}
				else
				{
					curr = ici.MaxBy(x => DecimalPart(x.Value)).First();
				}
				
				var added = Math.Ceiling((0.5 - DecimalPart(curr.Value)) / minPart);
				curr.Value += (int)added;
				if (added > remainingAnswers - i) break;
				if (curr.Value == 0) l++;
				ici.Remove(curr);
				alreadyOverHalf++;
				i += (int)added;
			...
			maxRoundedValue = (int)(ci.Sum(x => Math.Round(x.Value * 100.0 / n)));
		}

		private double DecimalPart(int c)
		{
			var x = c * 100.0 / n;
